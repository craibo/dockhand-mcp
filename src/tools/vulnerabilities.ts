...
export function registerVulnerabilityTools(server: McpServer, client: DockhandClient, readonly: boolean): void {
  ...
  server.registerTool(
    ...
    {
      description: 'Scan every image in an environment for vulnerabilities. Blocks until the batch scan completes.',
      inputSchema: z.object({
    ...
    async ({ environmentId }) => {
      try {
        const result = await client.post('/api/vulnerabilities/scan-all', undefined, { env: environmentId });
        return toTextResult(result);
      } catch (error) {
        return toErrorResult(error);
      }
